...
# Time Complexity
# The time complexity of the above algorithm is (n). Although we are not incrementing the index i when swapping the
# numbers, this will result in more than n iterations of the loop, but in the worst-case scenario, the while loop
# will swap a total of n-1 numbers, and once a number is at its correct index, we will move on to the next number by
# incrementing i. So overall, our algorithm will take O(n) + O(n-1) which is asymptotically equivalent to O(n).
#
# Space Complexity
