...
#   2. First, we will find the middle of start and end. An easy way to find the middle would be: middle=(
#      start+end)/2middle=(start+end)/2. For Java and C++, this equation will work for most cases, but when start or end
#      is large, this equation will give us the wrong result due to integer overflow. Imagine that end is equal to the
#      maximum range of an integer (e.g. for Java: int end = Integer.MAX_VALUE). Now adding any positive number to end
#      will result in an integer overflow. Since we need to add both the numbers first to evaluate our equation,
#      an overflow might occur. The safest way to find the middle of two numbers without getting an overflow is as
#      follows:
...
#   -If key > arr[middle], then we can conclude that the key will be greater than all numbers after index middle as the
#    array is sorted in the descending order. Hence, we can reduce our search to end = mid - 1.
#   -If key < arr[middle], then we can conclude that the key will be smaller than all the numbers before index middle as
#    the array is sorted in the descending order. Hence, we can reduce our search to start = mid + 1.
# Finally, how can we figure out the sort order of the input array? We can compare the numbers pointed out by start
# and end index to find the sort order. If arr[start] < arr[end], it means that the numbers are sorted in ascending
