...
#
# Notice that because we always evaluate the root node first in a preorder traversal, the first element in the preorder
# traversal will always be the root. The second element is then either the root of the left node if there is one, or the
# root of the right node. But how do we know?
#
# We can look at the inorder traversal.
#
# Because we look at the left node first in an inorder traversal, all the elements up until the root will be part of the
# left subtree. All elements after the root will be the right subtree.
#
# Preorder:
# [a, b, d, e, c, f, g]
# | r | left  | right |
