...
# This problem is quite a bit harder than Daily Coding Problem #47, where we only needed to buy and sell a stock once
# to get the max profit.
#
# Let's consider the last price in the array. Either we used that price in a transaction or we didn't. Thus,
# the max profit for our input should be the max of either:
#
...
# We can use dynamic programming to represent the maximum profit we can get. Let dp[i][j] represent the maximum
# profit using at most i transactions up to day j (inclusive).
#
...
#     max(price[j] - price[m] + dp[i - 1][m] for m in range(j)) # Best we can do by completing last transaction on jth
#     day )
...
#
# We can improve the time complexity by noticing that we're redoing some of the same calculations in the innermost
# for loop. In the term prices[j] - prices[m] + dp[i - 1][m], prices[j] stays constant and the rest of the term can
# be factored into the outer loop.
