...
#
# Solution
# There are two steps in solving this question: we must first parse the string representing the file system and then
# get the longest absolute path to a file.
#
...
#
# To achieve this, we can first split the string by the newline character, meaning each item in our array represents a
# file or directory. Then, we create an empty dictionary to represent our parsed file system and traverse the file system
# on each entry. We keep track of the last path we've seen so far in current_path because we may need to return to some level
# in that path, depending on the number of tabs. Once we are at the correct place to put down the new directory or file,
# we check the name for a . and set the correct value to either True (if file) or {} (if directory).
...
# This runs in O(n), since we iterate over the input string twice to build the file system, and then in the worst case
# we go through the string again to compute the longest path.