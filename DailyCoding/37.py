...
# If we're given a set with one element in it ({a}), then the power set is a set with two sets: an empty set and a set
# with the element in it: {{}, {a}}
# If we're given a set with two elements in it ({a, b}), then the power is has four sets: {{}, {a}, {b}, {a, b}}
# What's the pattern?
#
# Notice that going from the empty set to {a}, that we still keep the empty set in our result and have another set with
# a in it. Similarly, when going from one element to two, we keep the same result set with one element ({}, {a}), but we
# also have a duplicate set with the b in it ({b}, {a ,b}).
#
