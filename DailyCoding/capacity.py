...
#
# Solution
# Notice that the amount of water that can be filled up at a certain index i is the smaller of the largest height to the
# left and the largest height to the right minus the actual value at that point, because it will be trapped by the
# smaller of the two sides. So what we can do is to create two arrays that represent the running maximum heights,
# one from the left and one from the right. Then to count the total capacity, we can run through the both arrays and
# add up the smaller of the two arrays at that index.
...
# (since we know the largest element on the array is on the right). And then do a similar thing, but starting from the
# right side. So the general gist is this:
#
# Find the maximum element in the array -- let's say it's at index i
# Initialize a running maximum on the left to arr[0]
# Iterate from index 1 to i. At each step, update the running maximum if necessary and then increment a variable counter
# with the running maximum minus the value at that array.
