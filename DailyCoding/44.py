...
#
# Solution
# The brute force solution here should come naturally from the definition: we can run a doubly nested for loop over all
# pairs, and increment a counter whenever we encounter a larger element before a smaller element. That would look like this:
#
...
#                 count += 1
#     return count
# However, this would run in O(N^2), and we want something faster. We can use the following recursive, divide-and-conquer
# algorithm to count the number of inversions in O(N log N) time.
#
...
# If we are able to count all the inversions between A and B in linear time, then according to the master theorem for
