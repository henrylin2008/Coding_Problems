...
# To solve the first part, let's write a function group_lines that takes in all the words in our input sequence as well
# as out target line length k, and return a list of list of words that represents the lines that we will eventually justify.
# Our main strategy will be to iterate over all the words, keep a list of words for the current line, and because we want
# to fit as many words as possible per line, estimate the current line length, assuming only one space between each word.
# Once we go over k, then save the word and start a new line with it. So our function will look something like this:
...
def group_lines(words, k):
    ...
    groups.append(current_line)
    return groups
# Then, we'll want to actually justify each line. We know for sure each line we feed from group_lines is the maximum
# number of words we can pack into a line and no more. What we can do is first figure out how many spaces we have available
# to distribute between each word. Then from that, we can calculate how much base space we should have between each word
# by dividing it by the number of words minus one. If there are any leftover spaces to distribute, then we can keep track
# of that in a counter, and as we rope in each new word we'll add the appropriate number of spaces. We can't add more
# than one leftover space per word.
