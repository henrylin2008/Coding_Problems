...
#
# Solution
# We can obviously do this problem in linear time if we looked at each element in the array. However, we need to do it
# faster than linear time. A big clue should be that the array of integers was previously sorted, and then rotated. If
# it was just sorted, we could do a binary search. However, this array was also rotated, so we can't do a regular binary
# search. We can modify it slightly to get to where we want it, however.
#
...
# If the number we're looking at is larger than the first item in the array, then the rotation must occur later, so add
# dist; If not, then it must occur before, so subtract dist
...
def shifted_array_search(lst, num):
    ...
    while True:
        ...
        dist = dist // 2
# This solution runs in O(log n). However, this is definitely not the only solution! There are many other possible ways
# to implement this, but as long as you have the idea of doing binary search, you've got it.