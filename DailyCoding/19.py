...
#
# Solution
# The brute force solution here would be to generate all possible combinations of houses and colors, filter out invalid
# combinations, and keep track of the lowest cost seen. This would take O(N^K) time.
#
# We can solve this problem faster using dynamic programming. We can maintain a matrix cache where every entry [i][j]
# represents the minimum cost of painting house i the color j, as well as painting every house < i. We can calculate
# this by looking at the minimum cost of painting each house < i - 1, and painting house i - 1 any color except j, since
# that would break our constraint. We'll initialize the first row with zeroes to start. Then, we just have to look at the
# smallest value in the last row of our cache, since that represents the minimum cost of painting every house.
