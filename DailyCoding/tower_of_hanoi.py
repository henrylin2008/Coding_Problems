...
# The goal of the Tower of Hanoi is to get all n disks from the source peg to the target peg, using a spare peg and
# abiding by all the constraints.
#
...
# Now, let's assume we have an existing tower_of_hanoi function that can move n disks from a source peg to a target
# peg using a spare stack. The recurrence would then look like this:
#
...
# We're able to recursively move the disks because it doesn't break any constraints: we can just treat the base disk
...
# This will run in O(2n) time, since for each call we're recursively calling ourselves twice. This should also take
# O(n) space since the function call stack goes n calls deep.
