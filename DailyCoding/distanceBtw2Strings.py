...
#
# Solution
# First, notice that we can probably define this problem recursively. How can we notice this? If we look at the example
# (kitten -> sitting) and its solution path (kitten -> sitten -> sittin -> sitting), we can see that it's the minimum
# distance between sitten and sitting plus one.
#
# The recurrence, then, looks like this:
#
# If either s1 or s2 are empty, then return the size of the larger of the two strings (since we can trivially turn an
# empty string into a string by inserting all its characters)
...
# However, this runs very slowly due to repeated subcomputations. We can speed it up by using dynamic programming and
# storing the subcomputations in a 2D matrix. The index at i, j will contain the edit distance between s1[:i] and s2[:j].
