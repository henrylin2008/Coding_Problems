...
# the following recurrence to solve this problem:
#
...
#
# However, this algorithm runs in O(2^N) time, since we could potentially make two recursive calls each time. We can
# speed up using dynamic programming, as usual. We can either memoize our results so that we don't duplicate any work,
# or use a table and do bottom-up programming.
#
# Let's start with memoization. We can keep a cache and store all our results when we compute them in the cache. If we
# come across a string we've seen before, then we just need to look it up in the cache.
...
def make_palindrome(s):
    ...
    if s[0] == s[-1]:
        ...
        cache[s] = min(one, two)
        return min(one, two)
# However, this is inefficient due to buildup in the call stack. We can build a 2D table instead. We'll store, in each
# index, the shortest palindrome that can be made in the substring defined from i to i + j. Then instead of calling
# ourselves recursively, we'll just look up the values in our table:
...
def make_palindrome(s):
    ...
    return table[0][-1]
# Because we store a part of our input string in each index of our matrix, the time and space complexity for this
# solution is O(N^3).