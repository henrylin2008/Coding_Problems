...
# Iterate through both the clone and originals at the same time. For a given clone node, find the original's random
# clone counterpart in the hashmap, and set it as its random node. However, there's a clever way to use even less
# space.
#
# First, double the linked list by interleaving it with cloned nodes (without random set). For example, given 1 -> 2
# -> 3, becomes 1 -> 1 -> 2 -> 2 -> 3 -> 3. Set the cloned nodes' random by following the original, previous node's
# random.next. Restore the linked lists by separating them. For example, each original nodes need to set node.next =
# node.next.next.
def clone(node):
    node = double(node)
