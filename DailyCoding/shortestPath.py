...
#
# Solution
# The idea here is to use either BFS or DFS to explore the board, starting from the start coordinate, and keep track of
# what we've seen so far as well as the steps from the start until we find the end coordinate.
#
# In our case, we'll use BFS. We'll create a queue and initialize it with our start coordinate, along with a count of 0.
# We'll also initialize a seen set to ensure we only add coordinates we haven't seen before.
#
# Then, as long as there's something still in the queue, we'll dequeue from the queue and first check if it's our target
# coordinate -- if it is, then we can just immediately return the count. Otherwise, we'll get the valid neighbours of the
# coordinate we're working with (valid means not off the board and not a wall), and enqueue them to the end of the queue.
#
# To make sure the code doesn't get too messy, we'll define some helper functions: walkable, which returns whether or not
# a tile is valid, and get_walkable_neighbours which returns the valid neighbours of a coordinate.
