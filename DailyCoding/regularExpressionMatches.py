...
#
# Solution
# This problem should strike you as recursive. The string should match the regex if we can match the head of the string
# with the head of the regex and the rest of the string with the rest of the regex. The special characters . and * make
# implementing this a bit trickier, however, since the * means we can match 0 or any number of characters in the beginning.
#
...
# Otherwise, if the first thing in r is not proceeded by a *, then match the first character of both r and s, and if
# they match, return match(r[1:], s[1:]). If they don't, then return false.
# If the first thing in r _is_ proceeded by a *, then try every suffix substring of s on r[2:] and return true if any
# suffix substring works.
