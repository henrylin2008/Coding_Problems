...
#
# Alexandra's algorithm starts with a guess and iteratively improves until convergence. In each iteration, we improve our guess by averaging guess and n / guess. This formula comes from the fact that if guess is an overestimate, then n / guess would be an underestimate. For example, If n is 9, then a guess of 4 is an overestimate and 9 / 4 is an underestimate. On the other hand, if guess is an underestimate, then n / guess is an overestimate. The process converges when guess is3 which is equal to 9 / 3. For the full proof, please see here.
