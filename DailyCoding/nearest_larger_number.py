...
#
# Solution
# We can do this the brute force method by simply iterating over the range of the array, and trying each
# offset in both directions until we find a number larger than arr[i]:
...
#
# To speed up the querying, we can preprocess the array to build a lookup array. We'll store the closest index for
# each corresponding index in the array. To build it fast, we can simply look at pairwise elements along the array to
# see which one's bigger, and set the index of the lookup array appropriately.
#
...
#
# If the elements in the original array are distinct, then this will take linear time. Otherwise, if there are
# duplicates, then it could take O(n^2) time in the worst case.
