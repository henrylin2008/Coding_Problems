...
# airport, compute the person's itinerary. If no such itinerary exists, return null. If there are multiple possible itineraries,
...
#
# Solution
# This problem is similar to the N queens problem a few days ago: we have a desired final state (all the flights are used up),
# we can construct partial itineraries and reject them, and at each step we have potentially multiple avenues to explore.
...
# For each flight, temporarily add the destination to our itinerary and remove it from the flight list. Then call ourselves
# recursively with the new itinerary and flight list.
...
# To speed this up, we'll store all the flights into a dictionary with the origin as a key and a list of flight destinations
# from that origin as the value. Then we can look up our options in O(1) time instead of O(N) time.
