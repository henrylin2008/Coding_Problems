...
#
# Solution
# A binary tree is a data structure in which each node has at most two children (left / right). To solve
# this question, we can iterate over the tree to check nodes without children, also known as leaf nodes.
#
...
# We conveniently store the parent node for the path function, so we can find the ancestors to get their path to the
# root. We can initialize our binary tree with this code:
