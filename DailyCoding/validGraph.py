...
# k - 1, and then see if we can recursively paint the rest of the graph without any conflicting colors. We'll create a
# helper function valid(graph, colors) that looks at the last colored vertex and all its neighbours to see if it conflicts
# with any of its neighbours (i.e. has the same color). We can skip over all uncolored vertices here.
#
# To represent the colors, we can just keep a separate colors list that maps 1-to-1 with the vertices. You can also
# convert the graph into nodes and add a color property as well.
...
def colorable(graph, k, colors=[]):
    ...
    return False
# This runs in O(k^N) time and O(k) space, where N is the number of vertices, since we're iterating over k colors and
# we are backtracking over N vertices.