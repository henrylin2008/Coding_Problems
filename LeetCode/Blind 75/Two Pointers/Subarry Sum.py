...
# Note that in the implementation, typically we use prefix_sum[i] to denote the sum of elements in 0...i - 1 (rightmost
# element i is not included in the sum). One good thing about this is prefix_sum[0] then means sum of array up to but
# not including the first element, i.e. empty array. The definition of empty array sum is useful when there exists a
# subarray starting from 0 that sums up to the target. Without the definition of empty array sum we would miss it
