...
# Encoding the state
# How is this all useful to algorithms and dynamic programming? The answer is to encode states. Remember in
# backtracking problems like permutation, we used a used boolean array to keep track of state, i.e. which character
# has been used. If we were to memoize the state, we need to somehow serialize the array, e.g. turning it into a
# string which has extra computational cost. With a bitmask, the serialization from boolean array to integer is
# already done for you - it's just an integer. This reduces memory consumption and makes the algorithm faster and
# simpler.
#
# For instance, let us say that we are on a graph, and the problem requires us to remember all the nodes that we
# visited. We can encode each node as a boolean bit value as part of our bitmask and use that value as our dp state.

# In the example, we have visited node 1 and node 4. We encode that as 1001 which is equal to 2^0 + 2^3 = 9.
#
# Bitmask questions can vary and additional dp states may be required so the code provided will loosely follow what a
# bitmask type question will look like.
#
# Common bit manipulation operations
# Typically, these problems use a recursive function as well in order to do dp. Below is some pseudocode to outline
# this idea. Here are some useful operations to keep in mind:
