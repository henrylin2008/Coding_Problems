...
# Method 1: iterative | Breadth first search, go through nodes level by level, swap left and right nodes, and add it
# to the queue
# Time: O(n)
# Space: O(n)
...
def invertBinaryTree(tree):
    ...
    while len[queue]:   # if there's still node/s in the queue
        current = queue.pop(0)  # current node is first node in the queue
        if current is None:     # skip if the node is None
            continue
        swapLeftAndRight(current)   # helper function to swap left and right node
